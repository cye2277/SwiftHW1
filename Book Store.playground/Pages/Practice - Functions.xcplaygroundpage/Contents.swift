...
func totalBookPrice() -> Double {
    ...
    for i in 0...books.count-1 {
        totalBookPrice += Double(books[i]["price"]!)!
    }
...
func getBook(at index: Int) -> (title: String, author: String, price: Double)? {
    let indexOfBook: Int = index
    var titleOfBook: String = ""
    var authorOfBook: String = ""
    var priceOfBook: Double = 0.0
    if(indexOfBook < 9 ){
        titleOfBook = books[indexOfBook]["title"]!
        authorOfBook = books[indexOfBook]["author"]!
        priceOfBook = Double(books[indexOfBook]["price"]!)!
        return(titleOfBook,authorOfBook,priceOfBook)
    }
    return nil
}
